---
name: learn
description: Capture a specific lesson, gotcha, or pattern mid-session. Use when something surprising happens, an approach fails, a useful pattern emerges, or the user points out something important to remember.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(ls *)
---

# Capture Learning

Quickly record a lesson, gotcha, or pattern for future sessions. If the user named the
learning, use that; otherwise infer it from the conversation context.

## Process

1. **Classify the learning** from what the user wants to capture and the conversation context:
...
3. **Save it**:
   ...
   - **Decision**: Create a numbered ADR in `docs/decisions/` (one file per decision: context, decision, consequences).
   - **Skill**: Use the `manage-skill` skill to create it.
...
- If it's unclear what to capture, ask the user in one short question
